...
export default {
  ...
  methods: {
    ...
    async checkCart(cart) {
      //删除某一个产品的操作
      try {
        await this.$store.dispatch("checkCart", cart.skuId, cart.isChecked);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
    ...
    async updateChecked(cart,event) {
      // 要带给服务器的参数isChecked，不是布尔值应该是0|1
      try {
  ...
  computed: {
    ...
    //判断底部全选复选框是否勾选，（全部产品都选中，才勾选）
    isAllChecked() {
      return this.cartInfoList.every((item) => item.isChecked == 1); //如果item中isChecked全是1的话就返回true
    },
  },
};
</script>
